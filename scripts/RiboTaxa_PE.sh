...
echo "fastqc $DATA_DIR/"$SHORTNAME"_R1.$FORMAT  $DATA_DIR/"$SHORTNAME"_R2.$FORMAT  -dir $RESULTS -o $RESULTS/quality_control/before_fastqc" | tee /dev/fd/3

fastqc "$DATA_DIR"/"$SHORTNAME"_R1.$FORMAT  "$DATA_DIR"/"$SHORTNAME"_R2.$FORMAT  -dir $RESULTS -o "$RESULTS"/quality_control/before_fastqc
...
echo "bbduk.sh -Xmx${RAM}g \
	...
	out1=$RESULTS/quality_control/"$SHORTNAME"_1_noadapt.$FORMAT  \
...
echo "command line :" | tee /dev/fd/3
echo "bbduk.sh -Xmx${RAM}g \
	in1=$RESULTS/quality_control/"$SHORTNAME"_1_noadapt.$FORMAT  \
	in2=$RESULTS/quality_control/"$SHORTNAME"_2_noadapt.$FORMAT  \
	out1=$RESULTS/quality_control/"$SHORTNAME"_1_trimmed.$FORMAT  \
...
echo "command line :" | tee /dev/fd/3
echo "fastqc $RESULTS/quality_control/"$SHORTNAME"_1_trimmed.$FORMAT  $RESULTS/quality_control/"$SHORTNAME"_2_trimmed.$FORMAT  -dir $RESULTS -o $RESULTS/quality_control/after_fastqc" | tee /dev/fd/3
...
rm "$RESULTS"/quality_control/"$SHORTNAME"_1_noadapt.$FORMAT
rm "$RESULTS"/quality_control/"$SHORTNAME"_2_noadapt.$FORMAT    
...
echo "sortmerna --ref $SORTMERNA_DB/$SORTME_NAME.fasta,$SORTMERNA_DB/$SORTME_NAME \
	--reads $RESULTS/output_sortmerna/"$SHORTNAME"_mergedpaired.fastq \
	--fastx \
...
rm "$RESULTS"/output_sortmerna/"$SHORTNAME"_mergedpaired.fastq
...
echo "emirge_amplicon.py \
	-1 $RESULTS/output_sortmerna/"$SHORTNAME"_R1_16S18Sreads.fastq \
	-f $EMIRGE_DB/$REF_NAME \
	-b $EMIRGE_DB/$BWT_NAME \
	-l $MAX_LENGTH \
	-2 $RESULTS/output_sortmerna/"$SHORTNAME"_R2_16S18Sreads.fastq \
	-j $IDENTITY \
	-n $NUM_ITERATION \
	-i $MEAN_INSERT_SIZE \
	-s $STD_DEV \
...
echo "command line :" | tee /dev/fd/3
echo "bbmap.sh -Xmx${RAM}g in1=$RESULTS/quality_control/"$SHORTNAME"_1_trimmed.fastq \
...
join "$RESULTS"/SSU_sequences/"$SHORTNAME"_RA_length.tsv "$RESULTS"/SSU_sequences/"$SHORTNAME"_length_bySeq.tsv |awk '{ print $1,$4,$2,$3 }'|tr  ' ' \\t |sort -k1 |awk 'BEGIN{print "Seq_ID\tlength\treads_count\tRelative_abundance"}1' > "$RESULTS"/SSU_sequences/"$SHORTNAME"_Abundance.tsv
